      ...
      return (
        <div className="flex flex-wrap justify-center">
          {data.map((item, idx) => {
            ...
            return (
              <div key={idx} style={{ width: `${percentage * 2}px`, height: `${percentage * 2}px`, backgroundColor: getRandomColor() }} className="rounded-full m-1"></div>
            );
          })}
        </div>
      );
    case 'line':
      return (
        <div className="relative h-40 w-full">
          {data.map((item, idx) => (
            <div key={idx} className="absolute bottom-0 left-0 w-full flex justify-between">
              <div style={{ height: `${item.population / 5000000}px`, top: `calc(100% - ${item.population / 5000000}px)` }} className="w-1 bg-blue-500"></div>
              {idx < data.length - 1 && 
                <div className="absolute border-t-2 border-dotted border-gray-500" style={{ width: `${100 / (data.length - 1)}%`, left: `${(idx + 0.5) * 100 / (data.length - 1)}%`, top: `${item.population / 5000000}px` }}></div>}
            </div>
          ))}
        </div>
      );
  ...
  return (
    <div className="p-4 sm:p-8 bg-gray-100 min-h-screen">
      <Card className="max-w-lg mx-auto bg-white shadow-lg">
        ...
        <CardFooter>
          <p className="text-xs text-muted-foreground">Data is for demonstration purposes only.</p>
        </CardFooter>
      </Card>
    </div>
  );

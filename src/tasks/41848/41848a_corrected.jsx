...
import { format, getYear, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isSameDay, addMonths } from 'date-fns';
...
function Calendar() {
  ...
  const handleAddReminder = () => {
    if (selectedDate && reminderText) {
      const newReminders = { 
        ...reminders, 
        [selectedDate]: [...(reminders[selectedDate] || []), reminderText] 
      };
      ...
      <Card>
              ...
              {daysInMonth.map(day => (
                <div 
                  ...
                  className={`cursor-pointer text-center p-2 ${
                    getDay(day) === 0 || getDay(day) === 6 ? 'text-red-500' : ''
                  }`}
                >
                  <Day day={day} />
                </div>
        ...
        <CardFooter>
          <Button 
            ...
            disabled={getYear(addMonths(currentDate, 1)) > getYear(currentDate) + 4}
          >
            Next Month
          </Button>
        </CardFooter>
